...
function UserBoatsForm(props) {
    ...
    const toggleUserHandler = modelID => {
        ...
        const response = fetch(baseUrl + 'user/toggleInstructed/', authenticatedPostConfig(requestBody))
        .then(res => {
            if (!res.ok) {
                throw res
            }
                    ...
                    {loading
                        ...
                        : <>
                            {props.whereCrew.map( boat => {
                                    return <BoatFormElement boat={boat} user={user} toggleUserHandler={toggleUserHandler} key={boat.id}/>
                                }
                            )}
                       </>
                    }
